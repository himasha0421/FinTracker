...
export function ChatInterface() {
  ...
  const processTransactions = async (transactions: any[]) => {
    try {
      ...
      for (const transaction of transactions) {
        // Format the transaction data according to your schema
        const formattedTransaction = {
          ...
          accountId: 4, // Default account ID
        };

        console.log('Adding transaction:', formattedTransaction); // Debug log
      ...
      toast({
        title: 'Error',
        description: 'Failed to add transactions: ' + (error as Error).message,
        variant: 'destructive',
        duration: 5000,
      });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Process transactions if task_type is add_transactions
      if (data.task_type === 'add_transactions' && Array.isArray(data.data)) {
        console.log('Found transactions to process:', data.data); // Debug log
